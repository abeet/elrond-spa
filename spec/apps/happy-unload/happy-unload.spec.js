...
export default function() {
  describe(`happy-unload app :`, () => {
    ...
    beforeAll(() => {
      elrondSpa.registerApplication('./happy-unload.app.js', () => System.import('./happy-unload.app.js'), location => location.hash === activeHash);
    });
    ...
    beforeEach(done => {
      ...
      .then(app => myApp = app)
      .then(() => elrondSpa.unloadApplication('./happy-unload.app.js'))
    ...
    describe(`when waitForUnmount = false :`, () => {
      it(`unloads an app that is mounted, and then remounts it`, done => {
        ...
        .triggerAppChange()
        .then(() => {
          expect(elrondSpa.getAppStatus('./happy-unload.app.js')).toEqual('MOUNTED');
          ...
          return elrondSpa.unloadApplication('./happy-unload.app.js');
        })
        .then(() => {
          expect(elrondSpa.getAppStatus('./happy-unload.app.js')).toEqual('NOT_LOADED');
          expect(myApp.getNumBootstrapCalls()).toBe(1);
          expect(myApp.getNumMountCalls()).toBe(1);
      ...
      it(`unloads an app that isn't loaded, and then keeps it in NOT_LOADED status`, done => {
        ...
        .then(() => {
          expect(elrondSpa.getAppStatus('./happy-unload.app.js')).toEqual('NOT_MOUNTED');
          expect(myApp.getNumBootstrapCalls()).toBe(1);
          expect(myApp.getNumMountCalls()).toBe(1);
          expect(myApp.getNumUnmountCalls()).toBe(1);
          expect(myApp.getNumUnloadCalls()).toBe(0);
