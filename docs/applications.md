...
A elrond-spa registered application is everything that a normal SPA is, except that it doesn't have an HTML page.
In a elrond-spa world, your SPA contains many registered applications, where each has its own framework.
...
What determines if a registered application is mounted is its [activity function](/docs/elrond-spa-config.md#activity-function).
...
[register the application with elrond-spa](/docs/elrond-spa-config.md#registering-applications).
...
During the course of a elrond-spa page, registered applications are loaded, initialized (bootstrapped), mounted, unmounted, and unloaded.
elrond-spa provides hooks into each phase via `lifecycles`.

A lifecycle function is a function or array of functions that elrond-spa will call on a registered application.
...
- If elrond-spa is [not started](/docs/elrond-spa-api.md#start), applications will be loaded,
...
is available for many popular technologies. See [the ecosystem docs](/docs/elrond-spa-ecosystem.md) for details.

### Load
When registered applications are being lazily loaded, this refers to when the code for a registered application
is fetched from the server and executed. This will happen once the registered application's [activity function](/docs/elrond-spa-config.md#activity-function)
...
[activity function](/docs/elrond-spa-config.md#activity-function) returns a truthy value. When
...
[activity function](/docs/elrond-spa-config.md#activity-function) returns a falsy value. When
...
`unloaded`. This will not ever happen unless someone calls the [`unloadApplication`](/docs/elrond-spa-api.md#unload-application) API.
If a registered application does not implement the unload lifecycle, then it assumed that unloading the app is a no-op.

The purpose of the `unload` lifecycle is to perform logic right before a elrond-spa application is unloaded. Once
...
## Timeouts
By default, registered applications obey the [global dieOnTimeout configuration](/docs/elrond-spa-api.md#dieontimeout),
