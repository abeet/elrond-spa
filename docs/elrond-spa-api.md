# elrond-spa API
The elrond-spa library does not `export default`, but instead exports named functions and variables.
What this means is you can use the api in two ways:

```js
import * as elrondSpa from 'elrond-spa';
// OR
import {registerApplication, start} from 'elrond-spa';
```

## registerApplication
`registerApplication(name, whereToGetApplication, activeWhen)` is the most important api your single spa config will use.
It is described in detail inside of the [elrond-spa-config.md docs](/docs/elrond-spa-config.md#registering-applications)
...
- `NOT_LOADED`: the app has been registered with elrond-spa, but the app itself has not yet been loaded.
...
3. Trigger a reroute, during which elrond-spa will potentially mount the application that was just unloaded.
...
elrond-spa immediately unloads the specified registered application even if the app is currently mounted. If `true`, elrond-spa will unload
...
elrond-spa fires an event `elrond-spa:before-routing-event` on the window every time before a routing event occurs.
...
```js
window.addEventListener('elrond-spa:before-routing-event', () => {
	console.log('before routing event occurred!');
})
```

## routing event
elrond-spa fires an event `elrond-spa:routing-event` on the window every time that a routing event has occurred in which
elrond-spa verified that all apps were correctly loaded, bootstrapped, mounted, and unmounted.
This event will get fired after each hashchange, popstate, or triggerAppChange, even if no changes
to registered applications were necessary. Sample usage of this event might look like this:
```js
window.addEventListener('elrond-spa:routing-event', () => {
	console.log('routing event occurred!');
})
```

## app-change event
elrond-spa fires an event `elrond-spa:app-change` on the window every time that one or more apps were loaded, bootstrapped,
mounted, unmounted, or unloaded. It is similar to `elrond-spa:routing-event` except that it will not fire unless
...
```js
window.addEventListener('elrond-spa:app-change', () => {
	console.log(elrondSpa.getMountedApps())
})
```

## no-app-change event
When no apps were loaded, bootstrapped, mounted, unmounted, or unloaded, elrond-spa fires a `elrond-spa:no-app-change` event.
This is the converse of the `elrond-spa:app-change` event -- only one will be fired for each routing event.
```js
window.addEventListener('elrond-spa:no-app-change', () => {
	console.log(elrondSpa.getMountedApps())
})
```

## before-first-mount
Right before the first time that any app is mounted, elrond-spa fires a `elrond-spa:before-first-mount` event. This will happen
...
## first-mount
Right after the first time that any app is mounted, elrond-spa fires a `elrond-spa:first-mount` event. This event will only get fired once, ever.
...
## ensureJQuerySupport
`ensureJQuerySupport(jQuery)`: Since jquery does event delegation, elrond-spa
has to specifically monkey patch each version of jQuery that you're using. elrond-spa tries to do
this automatically as much as possible for looking for window.jQuery or window.$, but if you want
to give your version of jQuery to elrond-spa manually, call ensureJQuerySupport(jQuery). The
...
`setLoader(Loader)` sets the javascript [loader](https://whatwg.github.io/loader/) that will be used by elrond-spa.
...
calls are made. Once called, you may omit the [loading function](/docs/elrond-spa-config.md#loading-function) argument when
calling `registerApplication` and elrond-spa will assume that a registered application may be loaded with
