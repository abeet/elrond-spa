...
and a javascript file that registers elrond-spa applications. It is best practice to keep your
single spa config as small as possible and to simply defer to elrond-spa to manage
...
Instead, all of that functionality should be taken care of either by elrond-spa itself or by
a elrond-spa application.

## Index.html file
The main thing that you should be doing in your html file is executing your single spa config. For your
use case, this could mean something like `<script src="/elrond-spa-config.js"></script>`.
Example:
```js
<html>
  <body>
...
You must register [applications](/docs/applications.md) with elrond-spa so it knows how and when to 
...
The second argument to `registerApplication` must be either a function that returns a promise [loading function](/docs/elrond-spa-config.md#loading-function) or the resolved Application.
...
This allows you import the Application from another file or define applications inline in your elrond-spa-config
...
Another way of looking at this is that elrond-spa is a top-level router that has a lot of applications that have their own sub-router.

elrond-spa will call each application's activity function under the following scenarios:
- `hashchange` or `popstate` event
- `pushState` or `replaceState` is called
- [`triggerAppChange`](/docs/elrond-spa-api.md#triggerappchange) api is called on elrond-spa
- Whenever the `checkActivityFunctions` method is called

## Calling elrondSpa.start()
The [`start()` api](/docs/elrond-spa-api.md#start) **must** be called by your single spa config in order for 
...
```js
//elrond-spa-config.js
import { start } from 'elrond-spa';

/* Calling start before registering apps means that elrond-spa can immediately mount apps, without
