...
export function reroute(pendingPromises = [], eventArguments) {
  ...
  async function performAppChanges() {
    ...
    window.dispatchEvent(new CustomEvent("elrond-spa:before-routing-event", myCE));
  ...
  function finishUpAndReturn(callEventListeners=true) {
    ...
    try {
      const appChangeEventName = wasNoOp ? "elrond-spa:no-app-change": "elrond-spa:app-change";
      window.dispatchEvent(new CustomEvent(appChangeEventName));
      window.dispatchEvent(new CustomEvent("elrond-spa:routing-event"));
    } catch (err) {
      /* We use a setTimeout because if someone else's event handler throws an error, elrond-spa
   ...
   * waiting on elrond-spa. This includes haschange and popstate events for both
   * the current run of performAppChanges(), but also all of the queued event listeners.
   * We want to call the listeners in the same order as if they had not been delayed by
   * elrond-spa, which means queued ones first and then the most recent one.
   */
